...
В файле "Figure.py" Вы найдёте абстрактный класс геометрической фигуры, в папке ".\tests" - открытые тесты к программе (закрытое тестирование предварительно будем проводить мы), для запуска можно использовать pytest:
